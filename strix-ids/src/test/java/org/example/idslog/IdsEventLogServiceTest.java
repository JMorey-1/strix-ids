...
class IdsEventLogServiceTest {
  ...
  void addEvent_WhenMoreThanMaxEvents_ShouldKeepOnlyMostRecentOneHundred() {
    ...
    for (int i = 1; i <= 105; i++) {
      service.addEvent(IdsEventLevel.SCORE, "192.168.1." + i, 0.10, "Event " + i, new double[] {i});
    }
  ...
  void getRecentEvents_ShouldReturnCopyOfEventList() {
    ...
    // Add event
    service.addEvent(IdsEventLevel.SYSTEM, "system", null, "IDS service started", new double[0]);
