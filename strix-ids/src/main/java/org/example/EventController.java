 ...
 *
 * <p>This controller is the main entry point into the IDS. It takes each incoming request event,
 * turns it into a feature vector, then either collects it for training or scores it against the
 * trained model.
 */
@RestController
  ...
  public ResponseEntity<Void> receiveEvent(@RequestBody RequestEvent event) {
    ...
    if (anomalyDetectionService.isCollecting()) {
      ...
      idsEventLogService.addEvent(
          IdsEventLevel.COLLECT, event.getIp(), null, "Collecting training sample", features);
    ...
    if (anomalyDetectionService.isTrained()) {
      ...
      mitigationService.processDetectionEvent(
          event.getIp(), result.getLevel(), result.getMessage());
