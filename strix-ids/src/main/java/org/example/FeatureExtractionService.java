 ...
 *
 * <p>This service keeps a short rolling window of request events for each IP address. Instead of
 * judging a single request on its own, Strix looks at recent behaviour from the same IP and turns
 * that behaviour into numbers for the ML model.
 */
@Service
public class FeatureExtractionService {
   ...
   */
  private final ConcurrentHashMap<String, List<RequestEvent>> eventBuffer =
  ...
  private double[] calculateFeatures(List<RequestEvent> events) {
     ...
     */
    long postCount = events.stream().filter(e -> "POST".equalsIgnoreCase(e.getMethod())).count();
     ...
     */
    long adminCount =
        events.stream().filter(e -> e.getUri() != null && e.getUri().startsWith("/admin")).count();
    ...
    long uniqueEndpoints =
        events.stream().map(RequestEvent::getUri).filter(uri -> uri != null).distinct().count();

    // HTTP 401 usually means unauthorised.
    long unauthorizedCount = events.stream().filter(e -> e.getStatusCode() == 401).count();

    // HTTP 403 means forbidden.
    long forbiddenCount = events.stream().filter(e -> e.getStatusCode() == 403).count();

    // HTTP 404 can be useful for spotting endpoint scanning.
    long notFoundCount = events.stream().filter(e -> e.getStatusCode() == 404).count();

    // Count login attempts specifically.
    long loginAttemptCount = events.stream().filter(e -> "/auth/login".equals(e.getUri())).count();
    ...
    double failedLoginRatio =
        loginAttemptCount == 0 ? 0.0 : (double) failedLoginCount / loginAttemptCount;
