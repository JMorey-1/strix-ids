...
public class MitigationActionClient {
  ...
  private void sendAction(
      MitigationRecord mitigationRecord, MitigationActionType actionType, long expiresInSeconds) {
    MitigationActionRequest request =
        new MitigationActionRequest(
    ...
    try {
      ...
      idsEventLogService.addEvent(
          IdsEventLevel.SYSTEM, mitigationRecord.getIpAddress(), null, message, null);
    ...
    httpClient
        ...
        .exceptionally(
            error -> {
              String message =
                  "Failed to send " + actionType + " action to target app: " + error.getMessage();
