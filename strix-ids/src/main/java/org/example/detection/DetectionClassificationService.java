 ...
 *
 * <p>The ML model only gives Strix a score. This service adds the decision layer on top of that
 * score, deciding whether the traffic is normal, worth watching or serious enough to raise alert.
 * It also tries to explain the likely type of suspicious behaviour using the feature vector.
 */
@Service
public class DetectionClassificationService {
  ...
  private boolean isRepeatedFailedLogin(double[] features) {
     ...
     */
    return postRatio > 0.6 && loginAttemptRatio > 0.5 && failedLoginRatio > 0.7;
  }
  ...
  private boolean isEndpointScanning(double[] features) {
     ...
     */
    return uniqueEndpointRatio > 0.5 && (notFoundRatio > 0.15 || unauthorizedRatio > 0.25);
  }
  ...
  private boolean isAdminProbing(double[] features) {
     ...
     */
    return adminRatio > 0.5 && (unauthorizedRatio > 0.25 || postRatio > 0.4);
  }
