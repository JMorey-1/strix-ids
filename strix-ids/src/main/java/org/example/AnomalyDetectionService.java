 ...
 *
 * This service collects normal traffic feature vectors, trains an Isolation Forest model
 * and then scores new traffic against that trained model. It does not decide whether
 * something is an alert by itself. It only returns the anomaly score.
 */
@Service
        ...
        if (features.length != expectedFeatureLength) {
            throw new IllegalArgumentException(
                    "Unexpected feature vector length. Expected "
                            + expectedFeatureLength + " but got " + features.length
            );
        }
    }
}