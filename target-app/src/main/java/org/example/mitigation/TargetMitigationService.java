 ...
 *
 * <p>The IDS decides what action should be taken, but the target application is responsible for
 * enforcing that action against future requests.
 */
@Service
public class TargetMitigationService {
  ...
  public void applyMitigation(MitigationActionRequest request) {
    ...
    activeMitigations.put(ipAddress, new ActiveMitigation(actionType, expiresAtMillis));

    System.out.println(
        "[TARGET][MITIGATION] action="
            + actionType
            + " ip="
            + ipAddress
            + " reason="
  ...
  public boolean isRateLimitExceeded(String ipAddress) {
    ...
    Deque<Long> recentRequests =
        recentRequestsByIp.computeIfAbsent(ipAddress, key -> new ArrayDeque<>());
