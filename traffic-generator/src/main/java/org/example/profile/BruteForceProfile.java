 ...
 *
 * <p>This profile repeatedly sends failed login attempts against the authentication endpoint. It is
 * designed to create a clear suspicious pattern for Strix to detect through POST traffic, login
 * attempts and 401 responses.
 */
public class BruteForceProfile implements TrafficProfile {
  ...
  public void run(String ipAddress) throws Exception {
    ...
    for (int i = 0; i < BRUTE_FORCE_ATTEMPTS; i++) {
       ...
       */
      trafficClient.post(
          "/auth/login", "{\"username\":\"admin\",\"password\":\"attempt" + i + "\"}", ipAddress);
